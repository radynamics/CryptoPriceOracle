# CryptoPriceOracle
Fetchs exchange rates for currency pairs (eg. XRP/USD, BTC/EUR) from various sources. Data can be store in memory, MariaDB or PostgreSQL. For XRPL it's additionally possible to store exchange rates on-chain using an XRPL TrustSet command.
